    ...
    with col_left:
        with st.container(border=True):
            ...
            if not indices_df.empty:
                ...
                # 🔥 Feature 2: 공포탐욕지수 (VIX 기반 대체 게이지)
                vix_val = latest.get('^VIX', 20)
                fg_score = max(0, min(100, 100 - (vix_val - 10) * 2.5)) # VIX 10=100(탐욕), VIX 50=0(공포)
    ...
    with col_right:
        with st.container(border=True):
            ...
            if yc_data is not None and not yc_data.empty:
                ...
                fig_yc.update_layout(**THEME_LAYOUT, height=220)
...
# =====================================================================
# [6] 페이지 구성: 내 포트폴리오 관리 (10대 기능 풀패키지 적용)
# =====================================================================
def make_portfolio_page(acc_name):
    def page_func():
        # 🔥 Feature 9: 모바일 전용 간편뷰 토글
...
<div style='color:{C_DOWN}; font-size:1.6rem; font-weight:bold;'>{mdd_now:.1f}%</div>
                ...
                wait_msg = ""
                if is_wait and tgt_reg < app_reg: wait_msg = f"<br><br><span style='color:#ff9800;'>⏳ <b>상향 전환 대기 ({wait_d}/5일차):</b> 속임수를 피하기 위해 대기 중입니다.</span>"
                elif tgt_reg > app_reg: wait_msg = f"<br><br><span style='color:#e74c3c;'>🚨 <b>하락 전환 발동 대기:</b> 내일 즉시 하향 전환됩니다.</span>"

                if app_reg == 1: msg = f"**[R1: 완벽한 강세장]** 3배 레버리지를 가동해 수익을 캡처하십시오.{wait_msg}"
                elif app_reg == 2: msg = f"**[R2: 조정/경계]** 과도한 레버리지를 2배수 이하로 축소하십시오.{wait_msg}"
                elif app_reg == 3: msg = f"**[R3: 장기 하락장]** 하락 추세 컨펌. 레버리지 청산 후 GLD 방어.{wait_msg}"
                else: msg = f"**[R4: 시스템 패닉]** 주식 전량 매도 및 안전자산 대피.{wait_msg}"

                st.info(f"**현재 상태: R{app_reg} (체류 {dur}일째)**\n\n{msg.replace('<br>', '')}")
                ...
                ed_disp = st.data_editor(
                    ...
                    num_rows="dynamic", use_container_width=True, height=300, key=f"ed_{acc_name}",
                    column_order=["태그", "티커 (Ticker)", "수량 (주/달러)", "평균 단가 ($)", "현재가 ($)", "수익률 (%)"],
                ...
                if hist_dict and target_val > 0:
                    with st.container(border=True):
                        ...
                        fig_achieve.update_layout(**THEME_LAYOUT, height=300, hovermode="x unified", yaxis=dict(showgrid=True, ticksuffix="%"))
                        st.plotly_chart(fig_achieve, use_container_width=True)
                st.write("")

            elif block == "📝 매매 일지":
                st.markdown("#### 📝 매매 일지 & 시스템 로그")
                # 🔥 Feature 8: 매매일지 해시태그 & 검색기능
                ...
                with col_log2:
                    ...
                    if history:
                        ...
                        if search_q:
                            h_df = h_df[h_df['Log'].str.contains(search_q, case=False, na=False)]
