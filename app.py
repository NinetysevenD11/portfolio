...
def apply_custom_css():
    css_base = ""
    # 네이티브 컨테이너를 쓰기 위해 강제 높이, CSS 그리드는 모두 뺌 (글씨 잘림 방지)
...
# =====================================================================
# [6] 페이지 구성: AI 시스템 분석관 (글씨 잘림 해결 및 트렌디 네이티브 UI)
# =====================================================================
def page_ai_analyst():
    st.title("⚡ AI 시스템 분석관")
    ...
    # 🔥 네이티브 컨테이너 기반의 유연한 AI 분석 리포트 (글씨 잘림 100% 방지)
    ...
    if not c_df.empty:
        ...
        with st.container(border=True):
            ...
            st.caption("💡 **레짐 판단의 핵심 지표:** 나스닥(QQQ)이 200일 이동평균선(빨간선) 위에 있는지, 공포지수(VIX)가 25나 40을 넘었는지가 전략의 뼈대입니다.")
    else:
...
<div style='text-align:right; margin-top:5px; font-weight:bold;'>{progress_pct:.2f}%</div>
                ...
                # 🔥 HTML 박스 제거, st.metric을 사용한 트렌디 레이아웃
                c1, c2, c3, c4 = st.columns(4)
                with c1:
                    with st.container(border=True):
...
# 🔥 좌측 카테고리 (AI 시스템 분석관 분리 및 전략 명세서 복구 확인)
pages = {
    "시스템": [
